...
fn endpoint_match_arm(
    ...
    quote! {
        #endpoint_name_str if <Self::Api as dharitri_wasm::api::VMApi>::has_location(#location_tokens) =>
        {
...
pub fn generate_function_selector_body(contract: &ContractTrait) -> proc_macro2::TokenStream {
    ...
    quote! {
        if match fn_name {
            b"callBack" if <Self::Api as dharitri_wasm::api::VMApi>::has_location(dharitri_wasm::abi::EndpointLocationAbi::MainContract) => {
                self::EndpointWrappers::callback(self);
                return true;
            },
            b"init" if <Self::Api as dharitri_wasm::api::VMApi>::has_location(dharitri_wasm::abi::EndpointLocationAbi::ViewContract) => {
